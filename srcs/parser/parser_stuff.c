...
/*   Updated: 2022/03/22 21:54:43 by dtentaco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_ambient_light(t_scene *data, char **str)
{
	...
	data->amlight_color = ft_color_multiplication(&data->amlight_color,
			data->ambient_light);
}
...
void	parse_camera(t_scene *data, char **str)
{
	...
	if (data->cams != NULL)
		scene_error("Camera(C) can only be declared once in the scene\n");
	get_next(str);
...
void	parse_light(t_light **cs_light, char **str)
{
	...
	new_light->color = ft_color_multiplication(&new_light->color,
			new_light->br);
	*cs_light = new_light;
}
